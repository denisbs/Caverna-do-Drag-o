...
void marcarsaidas(){
    int i;
    for (i = 0; i< 3; i++)
        mapa[posicaoS[saidaMapa[i]][1]][posicaoS[saidaMapa[i]][0]] = posicaoS[saidaMapa[i]][2];
...
void* recebeClientes(void* arg){
    ...
    while(1)
    {
        ...
        if(personas[posicaoServidor].estado == 7){
            red.b = 7;
        }
        //printf("Posicao x %d\n",red[sockEntrada].x);
        //printf("Posicao y %d\n",red[sockEntrada].y);
...
int identiP(int x, int y){
    ...
    for (contador = 0; contador < 4; contador++ ){
            if(contador != posicaoServidor){
                        ...
                        if(x >= personas[contador].x && x<=(personas[contador].x+32) )
                            if(y >= personas[contador].y && y<=(personas[contador].y+40) )
                                coli =1;

                        if((x+32) >= personas[contador].x && (x+32)<=(personas[contador].x+32) )
                            if(y >= personas[contador].y && y<=(personas[contador].y+40) )
                                coli =1;

                        if((x) >= personas[contador].x && (x)<=(personas[contador].x+32) )
                            if((y+40) >= personas[contador].y && (y+40)<=(personas[contador].y+40) )
                                coli =1;
                        ...
                        if((x+16) >= personas[contador].x && (x+16)<=(personas[contador].x+32) )
                            if(y >= personas[contador].y && y<=(personas[contador].y+40) )
                                coli =1;
                        ...
                        if((x+16) >= personas[contador].x && (x+16)<=(personas[contador].x+32) )
                            if((y+20) > personas[contador].y && (y+20)<(personas[contador].y+40) )
                                coli =1;
       ...
       if(eixo == 1){
                 px += posix * SPEED;
                        ...
                        if( mapa[anaY][anaX] >= 35 && mapa[anaY][anaX] <= 57 && mapa[anaY][anaX] != 60 )
                         ...
                         if(mapa[anaY][anaX] > 70 && mapa[anaY][anaX] < 100){
                            if(red.chave == 1)
                            {
                                coli = 0;
                            }else{
...
// itens de informações de personagem Local
    ...
    ALLEGRO_BITMAP *chave = al_load_bitmap("C:/Inclusos/chave.png");
    if(!chave)
        error("faled to load chave");
    ...
    ALLEGRO_BITMAP *mheet = al_load_bitmap("C:/Inclusos/b.png");
    if(!mheet)
        error("failed to load mheet");

    ALLEGRO_BITMAP *pheet = al_load_bitmap("C:/Inclusos/p.png");
    if(!pheet)
        error("failed to load pheet");
    ...
    ALLEGRO_FONT *fonte2 = al_load_font("C:/Inclusos/aspartam.ttf", 40, 0);
    if (!fonte2)
        error("Falha ao carregar fonte.");


    ALLEGRO_BITMAP *sprites[4][NUM_SPRITES+4][NUM_SPRITES];

    int shift, shiftb;

    for(shift = 0; shift < (NUM_SPRITES+6); shift++){
        for(shiftb = 0; shiftb <NUM_SPRITES; shiftb++){
            sprites[0][shift][shiftb] = al_create_sub_bitmap(sheet, shiftb * SPRITE_SIZEH, shift * SPRITE_SIZEV, SPRITE_SIZEH, SPRITE_SIZEV);
        }
    }
    for(shift = 0; shift < (NUM_SPRITES+6); shift++){
        for(shiftb = 0; shiftb <NUM_SPRITES; shiftb++){
            sprites[1][shift][shiftb] = al_create_sub_bitmap(sheet, shiftb * SPRITE_SIZEH, shift * SPRITE_SIZEV, SPRITE_SIZEH, SPRITE_SIZEV);
        }
    }
    for(shift = 0; shift < (NUM_SPRITES+6); shift++){
        for(shiftb = 0; shiftb <NUM_SPRITES; shiftb++){
            sprites[2][shift][shiftb] = al_create_sub_bitmap(mheet, shiftb * SPRITE_SIZEH, shift * SPRITE_SIZEV, SPRITE_SIZEH, SPRITE_SIZEV);
        }
    }
    for(shift = 0; shift < (NUM_SPRITES+6); shift++){
        for(shiftb = 0; shiftb <NUM_SPRITES; shiftb++){
            sprites[3][shift][shiftb] = al_create_sub_bitmap(pheet, shiftb * SPRITE_SIZEH, shift * SPRITE_SIZEV, SPRITE_SIZEH, SPRITE_SIZEV);
        }
    }
...
// carregando imagens do mapa
  ...
  //armadilhas
    ...
    if(!t24)      error("failed to load trap 24");
    ...
    //SAIDAS
    ALLEGRO_BITMAP *s1 = al_load_bitmap("C:/Inclusos/mapa/saidas/s1.png");
    if(!s1)
      error("failed to load saida 1");
    ALLEGRO_BITMAP *s2 = al_load_bitmap("C:/Inclusos/mapa/saidas/s2.png");
    if(!s2)
      error("failed to load saida 2");
    ALLEGRO_BITMAP *s3 = al_load_bitmap("C:/Inclusos/mapa/saidas/s3.png");
    if(!s3)
      error("failed to load saida 3");
    ALLEGRO_BITMAP *s4 = al_load_bitmap("C:/Inclusos/mapa/saidas/s4.png");
    if(!s4)
      error("failed to load saida 4");
    ALLEGRO_BITMAP *s5 = al_load_bitmap("C:/Inclusos/mapa/saidas/s5.png");
    if(!s5)
      error("failed to load saida 5");
	ALLEGRO_BITMAP *s6 = al_load_bitmap("C:/Inclusos/mapa/saidas/s6.png");
    if(!s6)
      error("failed to load saida 6");
              ...
              switch(mapa[m][l]){
                  ...
                  case 1 :
					...
					case 134 :
						...
						break;
					case 71 :
						al_draw_bitmap(s1,(l*50)-cameraX,(m*50)-cameraY, 0);
						break;
					case 72 :
						al_draw_bitmap(s2,(l*50)-cameraX,(m*50)-cameraY, 0);
						break;
					case 73 :
						al_draw_bitmap(s3,(l*50)-cameraX,(m*50)-cameraY, 0);
						break;
					case 74 :
						al_draw_bitmap(s4,(l*50)-cameraX,(m*50)-cameraY, 0);
						break;
					case 75 :
						al_draw_bitmap(s5,(l*50)-cameraX,(m*50)-cameraY, 0);
						break;
					case 76 :
						al_draw_bitmap(s6,(l*50)-cameraX,(m*50)-cameraY, 0);
						break;
  ...
  /**********/
  // 4 = down, 1 = esquerda, 2 = direita, 3 = cima
    int ai;
  ...
  while(running) {
    ...
    case ALLEGRO_EVENT_KEY_DOWN:
      ...
      case ALLEGRO_KEY_SPACE:
            if(red.b < 3 && alguemganhou == 0){
                    setmove(6);
                    printf("direção: %d", direcao);
                    red.ataque = ataque(red.x, red.y, direcao);
    ...
    case ALLEGRO_EVENT_TIMER:
        ...
        if(event.timer.source == timer_mudanca_de_posicao_na_imagem_da_trap)
        {
            ...
            {
                pos_x_trap++;
                red.sprite = red.sprite+1;
                if(pos_x_trap == 4 && flag_trap == 0){

                        conf = identiP(red.xA,red.yA);
                        if(conf == 0){
                            red.x = red.xA;
                            red.y = red.yA;
                        }else{
        ...
        if(event.timer.source == timer_contador)
            {
                    maisumseg();
                    if(reloginho.segundos == 0 && reloginho.minutos == 1 ){
                        marcarsaidas();
                    }
         ...
         if(event.timer.source == timer_ataque)
         {
            if(personas[red.ataque].chave == 1){
                red.chave = 1;
                eu.chave = 1;
                personas[red.ataque].chave = 0;
                personas[servidor].chave = 1;
            }
    ...
    if(refresh) {
            ...
            if(red.b != 5 && alguemganhou != 1)
                {
                   ...
                   for (contador = 0; contador < 4; contador++){
                    //sortearNas(contador);
                           if(contador != servidor){
                                      al_draw_bitmap(sprites[contador][personas[contador].spritel][personas[contador].spritec], personas[contador].x-cameraX, personas[contador].y -cameraY, flags);
                           }

                            al_draw_bitmap(sprites[servidor][red.direcaoS][red.sprite], red.x -cameraX, red.y -cameraY, flags);
            ...
            else
        {
           ...
           atualizaCAM(red.x, red.y);
                ...
                for (contador = 0; contador < 4; contador++){
                    if(contador != servidor){
                        al_draw_bitmap(sprites[0][0], personas[contador].x -cameraX, personas[contador].y -cameraY, flags);
                    }else{
                        al_draw_bitmap(sprites[0][0], red.x-cameraX, red.y-cameraY, flags);
                    }
                }
                if(vencedor == servidor){
                    al_draw_textf(fonte2, al_map_rgb(0, 0, 0), 325, 250, ALLEGRO_ALIGN_CENTRE, "Parabens vc e um nerd!");
                }else{
                    al_draw_textf(fonte2, al_map_rgb(0, 0, 0), 325, 250, ALLEGRO_ALIGN_CENTRE, "O player %d ganhou, TROUXA!", vencedor);
                }
  ...
  al_destroy_bitmap(s1);
  al_destroy_bitmap(s2);
  al_destroy_bitmap(s3);
  al_destroy_bitmap(s4);
  al_destroy_bitmap(s5);
  al_destroy_bitmap(s6);



int shiftc;
  for( shiftc = 0; shiftc < 4; shiftc++)
    for(shift = 0; shift < NUM_SPRITES; shift++)
        for(shiftb = 0; shiftb < NUM_SPRITES; shiftb++)
            al_destroy_bitmap(sprites[shiftc][shift][shiftb]);
...
int criaCliente(){
 //   int winsock;
    ...
    sock.sin_addr.s_addr=inet_addr("10.135.108.93"/*"10.135.226.14"/*ip*/);
...
//    HANDLE th;
    ...
    for(o = 0; o<67; o++){
        for(p = 0; p<71; p++){
        recv(descritorCliente, &mapa[o][p], sizeof(mapa[o][p]),0);
        }
    }
